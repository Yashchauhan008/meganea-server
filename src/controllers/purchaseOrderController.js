...
export const getAllPurchaseOrders = asyncHandler(async (req, res) => {
    ...
    if (status) {
        if (status === 'QC') {
            query.status = { $in: ['QC_InProgress', 'QC_Completed'] };
        } else {
            query.status = status;
        }
    }
...
// ===== GENERATE PALLETS FROM PO =====
// FILE: backend/src/controllers/purchaseOrderController.js
// REPLACE THE generatePalletsFromPO FUNCTION WITH THIS

// ===== GENERATE PALLETS FROM PO (PARTIAL SUPPORT) =====
export const generatePalletsFromPO = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { itemId } = req.body; // Optional: generate for specific item only
    ...
    try {
        ...
        // Allow generation in QC_InProgress, QC_Completed, or Packing status
        const allowedStatuses = ['QC_InProgress', 'QC_Completed', 'Packing'];
        if (!allowedStatuses.includes(po.status)) {
            throw new Error(`Cannot generate pallets. PO status is '${po.status}', must be one of: ${allowedStatuses.join(', ')}`);
        }

        const palletsToCreate = [];
        const tileStockUpdates = new Map();
        ...
        if (itemsToProcess.length === 0) {
            throw new Error('No items found to generate pallets for.');
        }

        for (const item of itemsToProcess) {
            // Check if this item has passed QC for all ordered boxes
            if (item.quantityPassedQC < item.totalBoxesOrdered) {
                if (itemId) {
                    // If generating for specific item, throw error
                    throw new Error(
                        `Item "${item.tile?.name || 'Unknown'}" has not completed QC. ` +
                        `Passed: ${item.quantityPassedQC}/${item.totalBoxesOrdered} boxes.`
                    );
                } else {
                    // If generating for all items, skip this one
                    console.log(`Skipping item ${item.tile}: QC not complete (${item.quantityPassedQC}/${item.totalBoxesOrdered})`);
                    continue;
                }
            }

            // Check how many pallets/khatlis already generated for this item
            const existingPallets = await Pallet.countDocuments({
                sourcePurchaseOrder: po._id,
                tile: item.tile,
                type: 'Pallet'
            ...
            const palletsToGenerate = (item.palletsOrdered || 0) - existingPallets;
            const khatlisToGenerate = (item.khatlisOrdered || 0) - existingKhatlis;

            if (palletsToGenerate <= 0 && khatlisToGenerate <= 0) {
                if (itemId) {
                    throw new Error('All pallets/khatlis have already been generated for this item.');
                }
                continue; // Skip if already generated
            }

            // Calculate boxes being converted in this generation
            const boxesFromNewPallets = palletsToGenerate * po.packingRules.boxesPerPallet;
            const boxesFromNewKhatlis = khatlisToGenerate * po.packingRules.boxesPerKhatli;
            const totalBoxesConverting = boxesFromNewPallets + boxesFromNewKhatlis;

            const tileId = item.tile.toString();
            tileStockUpdates.set(tileId, (tileStockUpdates.get(tileId) || 0) + totalBoxesConverting);

            // Generate pallets
            for (let i = 0; i < palletsToGenerate; i++) {
                const palletId = await generateId('PA');
                palletsToCreate.push({
                    palletId,
                    factory: po.factory,
            ...
            // Generate khatlis
            for (let i = 0; i < khatlisToGenerate; i++) {
                const palletId = await generateId('PA');
                palletsToCreate.push({
                    palletId,
                    factory: po.factory,
            ...
            // Update item's generation tracking
            item.palletsGenerated = (item.palletsGenerated || 0) + palletsToGenerate;
            item.khatlisGenerated = (item.khatlisGenerated || 0) + khatlisToGenerate;
            item.boxesConverted = (item.boxesConverted || 0) + totalBoxesConverting;
        }

        // Insert all created pallets/khatlis
        if (palletsToCreate.length > 0) {
            const createdPallets = await Pallet.insertMany(palletsToCreate, { session });
            const palletIds = createdPallets.map(p => p._id);
            po.generatedPallets.push(...palletIds);
        ...
        const allItemsGenerated = po.items.every(item => {
            const palletsGenerated = item.palletsGenerated || 0;
            const khatlisGenerated = item.khatlisGenerated || 0;
            return palletsGenerated >= item.palletsOrdered && khatlisGenerated >= item.khatlisOrdered;
        });
        ...
        if (allItemsGenerated) {
            // All items fully generated
            po.status = 'Completed';
        } else if (po.status === 'QC_InProgress' || po.status === 'QC_Completed') {
            // Some items generated, move to Packing if not already
            po.status = 'Packing';
        }
